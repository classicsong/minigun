...
struct EdgeMax {
  ...
  static __device__ __forceinline__ void ApplyEdgeReduce(
    ...
    float* outbuf = gdata->max + outoff;

    while (tx < dim) {
      float *outaddr = outbuf + tx;
      *outaddr = max(*outaddr, gdata->score[gdata->eid_mapping[eid] * dim + tx]);
      tx += stride_x;
    }
  }
